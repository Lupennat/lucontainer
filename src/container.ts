...
class Container implements ContainerI {
    protected static instance: ContainerI | null = null;
     ...
     */
    public when(concrete: ContainerNewable | ContainerNewable[]): ContextualBindingBuilderI {
     ...
     */
    protected newContextualBinding(aliases: ContainerNewable[]): ContextualBindingBuilderI {
     ...
     */
    public bind<T>(abstract: ContainerAbstract<T>, concrete: ContainerConcrete<T> | null = null, shared = false): void {
    ...
    protected getClosure<T>(
        ...
        return ({ parameters }: { parameters: ContainerParameters }) => {
    ...
    public callMethodBinding(method: ContainerClassMethod, instance: any): any {
        ...
        return fnToCall({ instance, container: this });
    }
    ...
    public extend<T>(abstract: ContainerAbstract<T>, closure: ContainerExtendFunction<T>): void {
        ...
        if (this.instancesMap.has(abstract)) {
            this.instancesMap.set(abstract, closure({ instance: this.instancesMap.get(abstract), container: this }));
            this.rebound<T>(abstract);
        } else {
    ...
    public refresh<T>(abstract: ContainerAbstract<T>, target: any, method: string | symbol): T | undefined {
        return this.rebinding<T>(abstract, ({ instance }) => {
    ...
    protected rebound<T>(abstract: ContainerAbstract<T>): void {
        ...
        for (const fnToCall of this.getReboundCallbacks<T>(abstract)) {
            fnToCall({ instance, container: this });
        }
    }
     ...
     * Resolve the given type from the container.
     */
    protected resolve<T>(abstract: ContainerAbstract<T>, parameters: ContainerParameters = {}, raiseEvents = true): T {
        abstract = this.getAlias<T>(abstract);
        ...
        for (const extender of this.getExtenders(abstract)) {
            object = extender({ instance: object, container: this });
        }
     ...
     */
    protected getContextualConcrete<T>(abstract: ContainerAbstract<T>): ContainerConcrete<T> | void {
    ...
    public build<T>(concrete: ContainerConcrete<T>): any | undefined {
        ...
        if (typeof concrete === 'function' && !isFunctionConstructor(concrete)) {
            return (concrete as ContainerConcreteFunction<T>)({
                container: this,
    ...
    protected resolveDependencies(dependencies: ReflectionParameter[]): any[] {
        ...
        for (const dependency of dependencies) {
            ...
            if (!hasParameterNamedOverride && getParameterClass(dependency) === null) {
                if (this.hasParameterArrayOverride()) {
                    results.push(this.getParameterArrayOverride());
                    continue;
                }
            }
            const result =
     ...
     */
    protected hasParameterArrayOverride(): boolean {
        const parameters = this.getLastParameterOverride();
        if (Array.isArray(parameters)) {
     ...
     */
    protected getParameterArrayOverride(): any {
    ...
    protected resolvePrimitive(parameter: ReflectionParameter): any {
        ...
        if (concrete != null) {
            return unwrapIfClosure(concrete, { container: this });
        }
    ...
    protected fireBeforeCallbackArray(
        ...
        for (const fnToCall of callbacks) {
            fnToCall({ instance: abstract, parameters, container: this });
        }
    }
    ...
    protected fireCallbackArray(
        ...
        for (const fnToCall of callbacks) {
            fnToCall({ instance: object, container: this });
        }
    }
     ...
     */
    public static getInstance(): ContainerI {
     ...
     */
    public static setInstance(container: ContainerI | null = null): ContainerI | null {
